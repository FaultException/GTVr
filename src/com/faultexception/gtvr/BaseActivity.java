...
import com.faultexception.gtvr.ConnectionManager.ConnectionListener;
import com.faultexception.gtvr.TrackballHandler.Direction;
import com.faultexception.gtvr.TrackballHandler.Listener;
import com.faultexception.gtvr.TrackballHandler.Mode;
import com.faultexception.gtvr.protocol.ICommandSender;
import com.faultexception.gtvr.protocol.QueuingSender;
...
import com.faultexception.gtvr.widget.SoftDpad.DpadListener;
 ...
 * 
 */
public class BaseActivity extends CoreServiceActivity implements
		ConnectionListener {
			...
			public void run() {
				if (requestCode == CODE_SWITCH_BOX) {
					if (resultCode == RESULT_OK && data != null) {
						RemoteDevice remoteDevice = data
								.getParcelableExtra(DeviceFinder.EXTRA_REMOTE_DEVICE);
	...
	private void handlePairingResult(int resultCode) {
		...
		default:
			throw new IllegalStateException(
					"Unsupported pairing activity result: " + resultCode);
		}
	}
	...
	protected DpadListener getDefaultDpadListener() {
		return new DpadListener() {
			...
			public void onDpadMoved(SoftDpad.Direction direction,
					boolean pressed) {
	 ...
	 * @param pressed
	 *            {@code true} if the key was pressed
	 */
	...
	private final void showSwitchBoxActivity() {
		...
		startActivityForResult(DeviceFinder.createConnectIntent(this,
				getConnectionManager().getTarget(), getConnectionManager()
						.getRecentlyConnected()), CODE_SWITCH_BOX);
	}
	...
	private final void showPairingActivity(RemoteDevice target) {
		disconnect();
		if (target != null) {
			startActivityForResult(PairingActivity.createIntent(this,
					new RemoteDevice(target.getName(), target.getAddress(),
							target.getPort() + 1)), CODE_PAIRING);
		}
	}
	...
	private class MissingSenderToaster implements
			QueuingSender.MissingSenderListener {
	...
	private void logConnectionStatus(CharSequence sequence) {
		String message = String.format("%s (%s)", sequence, getClass()
				.getSimpleName());
