...
import com.faultexception.gtvr.protocol.AnymoteSender;
import com.faultexception.gtvr.protocol.DummySender;
import com.faultexception.gtvr.util.Debug;
import com.faultexception.gtvr.util.LimitedLinkedHashMap;
...
public final class CoreService extends Service implements ConnectionManager {
	...
	private enum State {
		IDLE, CONNECTING, CONNECTED, DISCONNECTING, DEVICE_FINDER, PAIRING
	}
	 ...
	 * @param ip
	 *            a string representing the ip of the target
	 * @param port
	 *            an integer representing the target's remote port
	 * @return {@code true} if the configuration is valid
	 */
	private static boolean isConfigValid(String name, String ip, int port) {
		return !INVALID_TARGET.equals(name) && !INVALID_IP.equals(ip)
				&& isPortValid(port);
	}
	...
	private void storeRemoteDevice(SharedPreferences.Editor prefEdit,
		...
		prefEdit.putString(DEVICE_IP_TAG + suffix, remoteDevice.getAddress()
				.getHostAddress());
	...
	private void loadConfig() {
		...
		for (int i = 0; i < getResources().getInteger(
				R.integer.recently_connected_count); ++i) {
	...
	private enum Request {
		CONNECT, CONNECTED, SET_TARGET, DISCONNECT, CONNECTION_ERROR, SET_KEEP_CONNECTED, REQUEST_PAIRING, PAIRING_FINISHED, REQUEST_DEVICE_FINDER, DEVICE_FINDER_FINISHED,
	}
	...
	private class ConnectionRequestCallback implements Handler.Callback {
		...
		private boolean changeState(State newState, Runnable callback) {
			if (isTransitionLegal(currentState, newState)) {
				if (Debug.isDebugConnection()) {
					Log.d(LOG_TAG, "Changing state: " + currentState + " -> "
							+ newState);
				}
		...
		private void handleSetKeepConnected(boolean keepConnected) {
			...
			if (Debug.isDebugConnection()) {
				Log.d(LOG_TAG, "KeepConnectedRefcount: "
						+ keepConnectedRefcount);
			}
		...
		private void sendNotification() {
			...
			case CONNECTED:
				connectionListener.onConnectionSuccessful(Debug
						.isDebugConnectionLess() ? new DummySender()
						: anymoteSender);
				break;
	...
	private static class ConnectionResult {
		...
		private ConnectionResult(ConnectionStatus status, AnymoteSender sender,
				Socket socket) {
			this.status = status;
			this.sender = sender;
			this.socket = socket;
	...
	private static class ConnectionTask extends
			AsyncTask<RemoteDevice, Void, ConnectionResult> {
		...
		protected ConnectionResult doInBackground(RemoteDevice... params) {
			if (params.length != 1) {
				throw new IllegalStateException(
						"Expected exactly one remote device");
			}
			...
			return new ConnectionResult(ConnectionStatus.ERROR_CREATE, null,
					null);
		}
		...
		private ConnectionStatus buildSocket(RemoteDevice target) {
			...
			try {
				...
				// Hack for Froyo which throws IOException for SSL handshake
				// problem:
			...
			// Connection successful - we need to reset connection attempts
			// counter,
		...
		private SSLSocket getSslSocket(RemoteDevice target)
			...
			// Create a new SSLContext, using the new KeyManagers and
			// TrustManagers
			...
			InetSocketAddress fullAddr = new InetSocketAddress(
	...
	private static Map<State, Set<State>> allowedTransitions() {
		...
		allowedTransitions.put(State.IDLE,
				EnumSet.of(State.IDLE, State.CONNECTING, State.DEVICE_FINDER));
		allowedTransitions.put(State.CONNECTING, EnumSet.of(State.CONNECTED,
				State.DEVICE_FINDER, State.PAIRING, State.DISCONNECTING));
		allowedTransitions
				.put(State.CONNECTED, EnumSet.of(State.DISCONNECTING));
		...
		allowedTransitions.put(State.DISCONNECTING,
				EnumSet.of(State.IDLE, State.CONNECTING));
