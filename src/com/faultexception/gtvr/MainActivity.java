...
import com.faultexception.gtvr.TouchHandler.Mode;
import com.faultexception.gtvr.layout.SlidingLayout;
import com.faultexception.gtvr.util.Action;
import com.faultexception.gtvr.widget.HighlightView;
import com.faultexception.gtvr.widget.KeyCodeButton;
import com.faultexception.gtvr.widget.SoftDpad;
...
public class MainActivity extends BaseActivity implements
	...
	private enum RemoteMode {
		TV(0, R.drawable.icon_04_touchpad_selector), TOUCHPAD(1,
				R.drawable.icon_04_buttons_selector);
	...
	/**
	 * Mode selector allow sliding across the modes, keeps currently selected
	 * mode information, and slides among the modes.
	 */
	private static final class ModeSelector {
		...
		ModeSelector(RemoteMode initialMode, SlidingLayout slidingLayout,
		...
		void slideNext() {
			setMode(RemoteMode.TOUCHPAD.equals(mode) ? RemoteMode.TV
					: RemoteMode.TOUCHPAD);
		}
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		ImageButton shortcutsButton = (ImageButton) findViewById(R.id.button_shortcuts);
		ImageButton liveTvButton = (ImageButton) findViewById(R.id.button_livetv);

		final ModeSelector current = new ModeSelector(RemoteMode.TV,
				slidingLayout, nextButton);
		...
		new TouchHandler(findViewById(R.id.touch_pad), Mode.POINTER_MULTITOUCH,
				getCommands());
		...
		if (intent != null) {
			if (Intent.ACTION_SEND.equals(intent.getAction())) {
				...
				if (text != null) {
					...
					if (uri != null
							&& ("http".equals(uri.getScheme()) || "https"
									.equals(uri.getScheme()))) {
						getCommands().flingUrl(text);
					} else {
						Toast.makeText(this, R.string.error_could_not_send_url,
	...
	private void onVoiceSearchResult(int resultCode, Intent data) {
		...
		ArrayList<String> queryResults = data
	...
	private void showVoiceSearchDialog(final String query) {
		...
		builder.setNeutralButton(R.string.voice_send,
				new DialogInterface.OnClickListener() {
					public void onClick(DialogInterface dialog, int which) {
						getCommands().string(query);
					}
				})
				.setPositiveButton(R.string.voice_search_send,
						new DialogInterface.OnClickListener() {
							public void onClick(DialogInterface dialog,
								...
								handler.postDelayed(
										...
										getResources().getInteger(
												R.integer.search_query_delay));
							}
						})
				.setNegativeButton(R.string.pairing_cancel,
						new DialogInterface.OnClickListener() {
							public void onClick(DialogInterface dialog,
									int which) {
							}
						}).setCancelable(true)
				.setTitle(R.string.voice_dialog_label).setMessage(query);
		builder.create().show();
	}
}
